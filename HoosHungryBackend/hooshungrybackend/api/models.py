...
class NutritionInfo(models.Model):
    ...
    def __str__(self):
        fields = []
        if self.calories is not None:          fields.append(f"Calories: {self.calories}")
        if self.protein is not None:           fields.append(f"Protein: {self.protein}g")
        if self.total_carbohydrates is not None: fields.append(f"Carbs: {self.total_carbohydrates}g")
        if self.cholesterol is not None:        fields.append(f"Cholesterol: {self.cholesterol}mg")
        if self.total_fat is not None:         fields.append(f"Total Fat: {self.total_fat}g")
        if self.saturated_fat is not None:     fields.append(f"Saturated Fat: {self.saturated_fat}g")
        if self.trans_fat is not None:         fields.append(f"Trans Fat: {self.trans_fat}g")
        if self.total_sugars is not None:      fields.append(f"Sugars: {self.total_sugars}g")
        if self.dietary_fiber is not None:     fields.append(f"Fiber: {self.dietary_fiber}g")
        if self.sodium is not None:            fields.append(f"Sodium: {self.sodium}mg")
